...
const engine = require(path.join(__dirname, "..", "play", "git-blocks.js"));
...
test("player files ship match-3 cabinet", () => {
  const gameDir = path.join(__dirname, "..", "play");
...
test("trophy catalog ships and facts stay off the board overlay", () => {
  ...
  const src = fs.readFileSync(path.join(__dirname, "..", "play", "git-blocks.js"), "utf8");
  ...
  const css = fs.readFileSync(path.join(__dirname, "..", "play", "git-blocks.css"), "utf8");
  assert.match(css, /\.trophy-case/);
  assert.match(css, /\.fact-toast[\s\S]*display:\s*none\s*!important/);
});

test("wordpress plugin packs the cabinet and stays synced with play/", () => {
  const plugin = path.join(__dirname, "..", "wordpress-plugin");
  const playJs = path.join(__dirname, "..", "play", "git-blocks.js");
  const playCss = path.join(__dirname, "..", "play", "git-blocks.css");
  ...
  assert.equal(
    ...
    fs.readFileSync(path.join(plugin, "assets", "js", "git-blocks.js"), "utf8")
  );
  assert.equal(
    fs.readFileSync(playCss, "utf8"),
    fs.readFileSync(path.join(plugin, "assets", "css", "git-blocks.css"), "utf8")
  );
